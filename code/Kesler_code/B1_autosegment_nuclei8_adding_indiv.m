...
for i = threshold_sampling/5:size(dapi_threshold2,2);                          %see how many nuclei for every threshold %
    ...
    dapi_bw_max = max(dapi_bw(:,:,1),[],3);                                           % Maximum projection for the binary pixels above the threshold
    if Yim == 1;
...
% % end
%% Adaptive cutoff
cell_nums = zeros(1,50);
for ik = 1:25                                                                     %This is a kind of cutoff. It's a percent of the maximum number of times a pixel is present 
DAPI_ims_final = DAPI_ims_added;
ik
if ik > 0
    DAPI_ims_final(find(DAPI_ims_final < max(DAPI_ims_final(:))*ik/100)) = 0;
...
dapi_label = bwlabeln(DAPI_ims_final,8);
cell_nums(ik) = max(dapi_label(:));
end
ik = find(cell_nums == max(cell_nums),1,'first');
DAPI_ims_final = DAPI_ims_added;
    DAPI_ims_final(find(DAPI_ims_final < max(DAPI_ims_final(:))*ik/100)) = 0;
    dapi_normal = bwareaopen(DAPI_ims_final, min_nucleus_size);                        % remove DAPI signal that are too small
    dapi_huge = bwareaopen(DAPI_ims_final, max_nucleus_size);                          % remove DAPI signal that are too large
