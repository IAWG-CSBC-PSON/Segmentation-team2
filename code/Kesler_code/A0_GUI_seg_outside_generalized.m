...
if handles.Diffstack
  ...
  handles.DAPI_prefix
  for i = 1:size(images2,2) 
      if not(isempty(strfind(images2{i},handles.DAPI_prefix)))
          handles.images_DAPI{counter_DAPI} = images1{i};
          im_prefixes{counter_DAPI} =  images2{i}(1:strfind(images2{i},handles.DAPI_prefix)-1);   %populate imprefixes (used for later segmentation file names)
          for j = 1:size(images2,2)
              if not(isempty(strfind(images2{j},handles.TRANS_prefix))) &...         %Look for TRANS prefix at beginning
                      strcmp(images2{j}(1:strfind(images2{j},handles.TRANS_prefix)-1),... %Look for the rest of image name to match between TRANS
                      images2{i}(1:strfind(images2{i},handles.DAPI_prefix)-1))        %and DAPI
...
if handles.Diffstack
  ...
  for i = 1:size(images2,2) 
      if not(isempty(strfind(images2{i},handles.DAPI_prefix)))
          ...
          for j = 1:size(images2,2)
              if not(isempty(strfind(images2{j},handles.TRANS_prefix))) &...         %Look for TRANS prefix at beginning
                      strcmp(images2{j}(1:strfind(images2{i},handles.TRANS_prefix)),... %Look for the rest of image name to match between TRANS
                      images2{i}(1:strfind(images2{i},handles.DAPI_prefix)))        %and DAPI
                      handles.images_TRANS{counter_DAPI} = images1{j}; 
              end
          end
          counter_TRANS = counter_TRANS+1;                                  %Add to counter (even if match not found)
...
% hObject    handle to SelSect_Sample_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
images1 = handles.images1;
if handles.Diffstack
