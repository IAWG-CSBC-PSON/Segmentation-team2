...
if strcmp(file_type,'Multiple')                                        % load images that are saved as image stacks
    ...
    if Diffstack
        ...
        DAPI_ims = imread(images1{im_num},Ych(2));
        TRANS_ims = imread(images1{im_num},Ych(3));
% ['D:\Segmentation\Harvard_Lung\LUNG-1-BR\LUNG-1-BR_40X_' num2str(im_num) '.tif']
%         DAPI_ims = imread(['D:\Segmentation\Harvard_Lung\LUNG-2-BR\LUNG-2-BR_40X_' num2str(im_num) '.tif']);
...
%      end;
    ...
    if false; %exist([outfile_prefix_seg fileTRANS],'file');
        ...
        [cells,plane,CellInfo,trans_plane,CellImage]= B2_autosegment_cells_new_yeast(TRANS_ims,segment_mode,Yim,min_cell_size,max_cell_size,dapi_label); % segment the trans image    
        ...
        [cells,plane,CellInfo,trans_plane,CellImage]= B2_autosegment_cells_new(TRANS_ims,segment_mode,Yim,min_cell_size,max_cell_size,dapi_label,img_stacks); % segment the trans image    
        save([outfile_prefix_seg fileTRANS],'cells','plane','segment_mode','CellInfo','trans_plane','CellImage','-mat','-v7.3'); % save the segmented cells
        end
    end;
    else;
